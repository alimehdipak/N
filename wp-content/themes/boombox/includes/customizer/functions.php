	...
	if ( ! $data ) {
		...
		if ( is_single() ) {
			$category_ids = wp_get_post_categories( get_the_ID(), array(
				...
				'meta_query' => array(
					...
					array(
						'key'     => 'boombox_term_logo_dark',
						'value'   => '',
						'compare' => '!='
					),
					array(
						'key'     => 'boombox_term_logo_dark_hdpi',
						'value'   => '',
						'compare' => '!='
					)
				)
			...
			if ( ! empty( $category_ids ) ) {
				...
				$term_logo_dark      = boombox_get_term_meta( $cat_id, 'boombox_term_logo_dark' );
				$term_logo_dark_hdpi = boombox_get_term_meta( $cat_id, 'boombox_term_logo_dark_hdpi' );
				...
				$set = array(
					...
					'branding_logo_dark'      => $term_logo_dark ? wp_get_attachment_url( $term_logo_dark ) : '',
					'branding_logo_dark_hdpi' => $term_logo_dark_hdpi ? wp_get_attachment_url( $term_logo_dark_hdpi ) : '',
		...
		if ( ! empty ( $set['branding_logo'] ) || ! empty ( $set['branding_logo_hdpi'] ) ) {
			...
			if ( ! empty ( $set['branding_logo_hdpi'] ) ) {
				$data['light']['src_2x'][] = $set['branding_logo_hdpi'] . ' 2x';
			}

			if ( ! empty ( $set['branding_logo'] ) ) {
				$data['light']['src']      = $set['branding_logo'];
				$data['light']['src_2x'][] = $set['branding_logo'] . ' 1x';
			} else {
				$data['light']['src'] = $set['branding_logo_hdpi'];
			}

			$data['light']['src_2x'] = implode( ',', $data['light']['src_2x'] );

		}

		if ( ! empty ( $set['branding_logo_dark'] ) || ! empty ( $set['branding_logo_dark_hdpi'] ) ) {
			$logo_data['dark'] = array();

			if ( ! empty ( $set['branding_logo_dark_hdpi'] ) ) {
				$data['dark']['src_2x'][] = $set['branding_logo_dark_hdpi'] . ' 2x';
			}

			if ( ! empty ( $set['branding_logo_dark'] ) ) {
				$data['dark']['src']      = $set['branding_logo_dark'];
				$data['dark']['src_2x'][] = $set['branding_logo_dark'] . ' 1x';
			} else {
				$data['dark']['src'] = $set['branding_logo_dark_hdpi'];
			}

			$data['dark']['src_2x'] = implode( ',', $data['dark']['src_2x'] );
