	...
	if ( ! class_exists( 'Boombox_Amp' ) ) {
		...
		class Boombox_Amp {
			...
			public function is_amp() {
				return function_exists( 'amp_is_request' ) ? amp_is_request() : false;
            }
...
//				todo Uncomment when hooks will work in the correct order, at this moment "amp_post_template_metadata" works before "amp_post_template_data"
//				$boombox_logo      = ! empty( $this->customizer_settings['boombox_logo'] ) ? $this->customizer_settings['boombox_logo'] : '';
//				$boombox_logo_hdpi = ! empty( $this->customizer_settings['boombox_logo_hdpi'] ) ? $this->customizer_settings['boombox_logo_hdpi'] : '';

				$customizer_settings = AMP_Customizer_Settings::get_settings();
				$boombox_logo        = ! empty( $customizer_settings['boombox_logo'] ) ? $customizer_settings['boombox_logo'] : '';
				$boombox_logo_hdpi   = ! empty( $customizer_settings['boombox_logo_hdpi'] ) ? $customizer_settings['boombox_logo_hdpi'] : '';

				$logo = $this->get_logo( (int) $boombox_logo, (int) $boombox_logo_hdpi );
			...
			/**
			 * Render post formatted time
			 * @param null $post
			 */
			public function render_post_time_tag( $post = null ) {
				$type = apply_filters( 'boombox/amp/post_date_type', 'published' );
				...
				if ( 'difference' == $display_format ) {
					switch ( $type ) {
						case 'modified':
							$difference = human_time_diff( get_the_modified_time( 'U', $post ), current_time( 'timestamp' ) );
							break;
						case 'published':
							$difference = human_time_diff( get_the_time( 'U', $post ), current_time( 'timestamp' ) );
							break;
						default:
							$difference = false;
					}
					...
					switch ( $type ) {
						case 'modified':
							$date = date_i18n( $format, get_the_modified_time( 'U', $post ) );
							break;
						case 'published':
							$date = date_i18n( $format, get_the_time( 'U', $post ) );
							break;
					}
				}
