...
// Not Safe For Work
$is_nsfw = boombox_is_nsfw_post( $this->get( 'post_id' ) );
...
<main>
    ...
    <article class="container post m-b-lg">
        ...
        <!-- Post Featured Image -->
        <?php
        if ( apply_filters( 'boombox/single/show_media', ( $this->get( 'boombox_template_options' )->media && boombox_show_multipage_thumbnail() && ( boombox_has_post_thumbnail() ) ) ) && ! $protect_content ) {
		...
		<div class="post-content">
		    <?php if ( $protect_content ) { ?>
				<figure class="s-post-thumbnail post-thumbnail bb-mb-el">
					<a href="<?php echo esc_url( get_permalink() ); ?>" class="entry-nsfw js-authentication"><?php echo boombox_get_nsfw_message(); ?></a>
				</figure>
		    <?php } else {
